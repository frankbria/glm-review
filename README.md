...
- Two limits bound a review: `--max-turns 80` and a 30-minute timeout on the review *step*, with the job capped at 35 minutes as an outer backstop. The turn cap is the one meant to bind. Both fail the step rather than the job on purpose — a job-level timeout cancels every remaining step, `if: always()` included, so nothing downstream of it can report what happened.
- A review that does not finish rewrites its own tracking comment into an explicit **"GLM review did not complete"** notice naming the reason (turn cap, step timeout, mid-run error). The "in progress" stub is never the final state, so *no review* can never be mistaken for *no defects found*.
