...
- Two limits bound a review: `--max-turns 60` and a 30-minute timeout on the review *step*, with the job capped at 35 minutes as an outer backstop. The turn cap is the one meant to bind — turns cost roughly 22s each, so 60 lands near 22 minutes, under the step clock rather than beside it. The clock is on the step, not the job, on purpose: a job-level timeout cancels every remaining step, `if: always()` included, so nothing downstream of it could report what happened.
